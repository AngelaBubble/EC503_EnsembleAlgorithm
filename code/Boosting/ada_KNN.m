...
for iter = 1 : M
    ...
    train_ada_knn       = sign(sum(train_predict * beta, 2));
    train_ada_CCR(iter) = 1- sum(train_ada_knn ~= trlabel1) / length(trlabel1);
    test_ada_knn        = sign(sum(test_predict * beta, 2));
    test_ada_CCR(iter)  = 1- sum(test_ada_knn ~= telabel1) / length(telabel1);
...
clear test_ada_knn test_data test_label 
clear train_ada_knn train_data train_label train_set
...
ylabel('CCR')
title('Adaboosting with KNN as Weak Learner')
hold off
