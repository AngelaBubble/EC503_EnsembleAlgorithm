...
for iter = 1 : M
    ...
    lr_mdl = fitclinear(X,Y,'Learner','logistic');
    h_hat(:, iter) = predict(lr_mdl, X);
    ...
    % final vote
    tmp_predict = predict(lr_mdl, X);
    train_predict(:,iter) = predict(lr_mdl, train_data1);
    test_predict(:,iter)  = predict(lr_mdl, test_data1);
    
    % calculating Adaboosting CCR for each iteration
    tmp_CCR(iter) = 1- sum(tmp_predict ~= Y) / length(Y);
    train_ada_lr      = sign(sum(train_predict * beta, 2));
    train_ada_CCR(iter) = 1- sum(train_ada_lr ~= trlabel1) / length(trlabel1);
    test_ada_lr        = sign(sum(test_predict * beta, 2));
    test_ada_CCR(iter)  = 1- sum(test_ada_lr ~= telabel1) / length(telabel1);
...
ylabel('CCR')
title('Adaboosting with Logistic Regression as Weak Learner')
hold off
